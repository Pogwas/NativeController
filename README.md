...
|---|---|
| `Gamepad` | Master toggle, right-stick look speed (X/Y), invert X/Y, stick deadzone, menu cursor speed, toggle sprint/grab/crouch, glyph style override (Auto / Xbox / PlayStation / Switch), `PushToTalkButton` (default `None`) — hold to talk when the game's Push to Talk setting is on, `HideMouseCursor` (default `true`) — no idle menu pointer while the pad is active |
...
| `Voice Indicator` | Muted-mic icon while your mic is cold in Push to Talk mode (clears while you hold the talk button): `Enabled` |
